//Variable containing the spanish translations of all the elements and descriptions.
var esp = {
  e1:"Hidrógeno",
  e2:"Helio",
  e3:"Litio",
...
//Variable containing the english translations of all the elements and descriptions.
var eng = {
  e1:"Hydrogen",
  e2:"Helium",
  e3:"Lithium",
...
//creates an array with all the elements in the HTML that have the "element" id. It stores them with their properties so they can be adressed like this: elarr[x].id
var elarr = document.getElementsByClassName("element");

//variables that are given values as arrays containing the keys or values of those keys for all the language variables (eng, esp, names). q = [e1,e2,e3...], w = [emptystring, emptystring,...], z1 = [Hydrogen, Helium, Lithium,...].
...
function goeng(){
  //Sets the content of the "lowercontent" div to be the value of the textoeng variable.
...
function goesp(){
  //Sets the content of the "lowercontent" div to be the value of the textoesp variable.
  document.getElementById("lowercontent").innerHTML = textoesp;
    //This for() loop goes through the values of the y1[] array and compares them with the ids of the elements of elarr[] and then to the elements of the w[] array. If the value of any item in the y1 array coincides with the id of any element inside elarr[], the innerHTML of that element is set to the value of the same index (i) of a third array z2[] which contains the spanish translations z2 = [Hidrógeno, Helio, Litio,...]. In the second for() loop, if any element of w[] is equal to any element of y1[], which will always happen as they contain the same key names in the same order, then the q[] array value of that same index (k) will be set to contain the value of z2[i] meaning it will start containing the spanish translation  of that word.

    //All is left to do now is to set the text of any variable to q[0], q[1], q[2]... etc. In the goeng() function we do the same thing but instead of setting values of q[] as the values of the esp variable key values, we set them as values of the eng variable key values which are contained in z1[].
    for(var i = 0; i < y1.length; i++) {
    for(var j = 0; j < elarr.length; j++){
      if(elarr[j].id == y1[i]){
...
<dt>Q: Te gusta la tortilla de patata con o sin cebolla?</dt>
