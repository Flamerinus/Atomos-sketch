...
//variables containing the number of protons, neutrons and electrons, as well as radii and angles, for clarity purposes only, the act() function sets the values for most of them from frame 1
	...
	//if the number of electrons set by the act() function is less or equal than 2 the for() loop  makes the electrons spread in a 360º arc uniformly using sin and cos functions starting at the atom's x and y, with the radius of the 1st shell being "radiopequeelectron" and the angle being "angulorealelectron" which gets increased every cycle of the for() loop by "anguloelectron" which is set by the act() function depending on the number of total electrons in that shell, then pushes a new Electron object in the electro1 array on the 1st shell every cycle until the number of elements reaches 2.
		if(atomo.electrones <= 2) {
			for(i = 0; i < atomo.electrones; i++) {			
				...
				//this array is declared in the act() function right before this function is called.It takes, x and y coordinates as well as the nubmer of the shell the electron is to be created on.
		...
		//The same thing is done with the protons, but instead of 3 shells, they are divided in two imaginary shells inside the nucleus.The 1st smaler one takes up to 8 protons and the rest go in a second bigger shell, around the small one.		
		if(atomo.protones <= 8) {
	...
	//Function that is triggered in the 1st frame(in the setup() main function of the p5.js library, down below) and every time one of the buttons is pressed.It takes the argument "numeroelem" from each of the buttons, and uses it to deremine the atom to display, from Hidrogen to Argon.	
	act : function(numeroelem) {
			//this if() statement makes sure that if the selected element is Hidrogen, the one proton in the nucleus gets displayed in the center, and when there are 2 or more protons present, they get displayed at a distance from the center, in an imaginary shell with this radius: "radiopequeproton".
			if(numeroelem === 1){
			   atomo.radiopequeproton = 0;
			   } else {
				   atomo.radiopequeproton = 15;
			   }
		//this is the main part of the function, where it checkes to see what button has been pressed and changes the variables to display the correct number of protons, neutrons and electrons, as well as the correct atomic weight, name, symbol and angles for the uniform spread of the protons and electrons in the crear() function, depending of the amount and the shell they are in.
			if(numeroelem === 1) {
				atomo.neutrones = 0;
...
function Electron(x, y, c) {
	...
	this.mover = function() {		
		...
		if(this.capa === 1) {
			...
			this.x = this.x + sin(this.angulorot) * this.radio;
			this.y = this.y + cos(this.angulorot) * this.radio;
			//This non-existent shell 0 is used solely for the purpose of having the one electron in the lower left corner bouncing around.As you can see it uses no sin/cos functions because it is not spinning, but bouncing inside a tiny boouncing box.The way the if() statement works is: it checks to see if the x coordinate of this instance of the electron: "this.x" is greater than another instance of the x we stored a second time a few lines of code ago, which we wil use as an origin: "this.fixx" + 1 or if "this.x" is smaller than the same "this.fixx" - 1.If either is true, than it multiplies the speedx variable of this instance of the electron "this.speedx" by -1, which has the effect of making it negative if it's positive and making it positive if it's negative.Then after it does the same with the y coordinate it just adds the speedx to the x and the speedy to the y basically moving the electron's x and y coordinates inside a bouncing box which is 4x4, and whenever it's this.x is greater than the original x "this.fixx" +1 which means this.fixx +2, it makes it move in the opposite direction, until it gets to be smaller than this.fixx -1 which means this.fixx -2 and then it makes it go the other way again.The same hapens with the y coordinate. The speedx and speedy can be changed, of course, and so can the +1 and -1 for the this.fixx and this.fixy, in order to make the bouncing box bigger or smaller.This is the exact procedure used to make the protons and neutrons bounce.
		} else if(this.capa === 0) {
...
function elembox() {
	...
	//This code is for the upper right corner of the box, where the electron configuration is displayed.If the number is greater than 10 then it displays the first shell's 2 electrons and the second shell's 8 electrons always, and then it takes the number of electrons the element has, subtracts the 10 already being displayed and shows it below. So Mgnesium shows 2,8,2 = 2,8 and 12-10.
...
function setup() {
	...
	angleMode(DEGREES);
	//This just triggers the act() function in the same way pressing the Hidrogen button would.This is done so that the canvas displays something before any button gets clicked, and the canvas doesn't appear empty.
	atomo.act(1);	
}
